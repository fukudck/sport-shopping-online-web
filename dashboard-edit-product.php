...
require("admin/php/edit-product-view.php");
          ...
          <section class="col-lg-8 pt-lg-4 pb-4 mb-3">
            <div class="pt-2 px-4 ps-lg-0 pe-xl-5">
              ...
              <div class="d-sm-flex flex-wrap justify-content-between align-items-center pb-2">
                <h2 class="h3 py-2 me-2 text-center text-sm-start">Sửa Sản Phẩm</h2>
              </div>
              <form action="admin/php/editProduct.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3 pb-2">
                  <div class="py-2">
                    ...
                    <select class="form-select me-2" id="category" name="category">
                      ...
                      displayCategoryOptions($categories, null, 0, $product["category_id"]);
                      ?>
                      <input type="hidden" name="old_category_id" value="<?php echo $product['category_id']; ?>">
                    </select>
                  </div>
                </div>
                <div class="mb-3 pb-2">
                  ...
                  <select id="sizeType" class="form-control" name="sizeType">
                    <option value="none" <?= $sizeType === "none" ? 'selected' : '' ?>>Không có</option>
                    <option value="letter" <?= $sizeType === "letter" ? 'selected' : '' ?>>Theo chữ</option>
                    <option value="number" <?= $sizeType === "number" ? 'selected' : '' ?>>Theo số</option>
                  </select>
                </div>

                <!-- Input số lượng khi không có size -->
                <div id="noSize" class="size-options"
                  style="<?= $sizeType === 'none' ? 'display: block' : 'display:none' ?> ">
                  ...
                  <div class="input-group mb-2" style="max-width: 200px;">
                    ...
                    <input type="number" class="form-control" name="total_quantity" min="0"
                ...
                <div id="letterSizes" class="size-options"
                  style="<?= $sizeType === 'letter' ? 'display: block' : 'display:none' ?>">
                    ...
                    <?php foreach (['XS', 'S', 'M', 'L', 'XL'] as $size): ?>
                      <div class="col-6 col-md-4 mb-2">
                        <div class="input-group">
                          ...
                          <input type="number" class="form-control" name="size[<?= $size ?>]" min="0"
                            value="<?= $sizes[$size] ?? 0 ?>">
                        </div>
                      </div>
                    <?php endforeach; ?>
                ...
                <div id="numberSizes" class="size-options"
                  style="<?= $sizeType === 'number' ? 'display: block' : 'display:none' ?>">
                    ...
                    <?php for ($i = 39; $i <= 50; $i++): ?>
                      <div class="col-6 col-md-4 mb-2">
                        <div class="input-group">
                          ...
                          <input type="number" class="form-control" name="size[<?= $i ?>]" min="0"
                            value="<?= $sizes[$i] ?? 0 ?>">
                        </div>
                      </div>
                    <?php endfor; ?>
                ...
                <div id="file-list" style="display: flex; gap: 10px; flex-wrap: wrap;">
                  <?php foreach ($images as $image_url): ?>
                    <img src="<?= htmlspecialchars($image_url); ?>" style="width: 100px; height: auto;"
                      alt="Product Image">
                  <?php endforeach; ?>
                </div>
                <script>
                ...
                <button class="btn btn-primary d-block w-100" name="editProduct" type="submit"><i
                    class="ci-cloud-upload fs-lg me-2"></i>Cập nhập</button>
              </form>
            </div>
          </section>
        </div>
